...
クライアントは`continuation`の内容を解釈せず、`cont`として送信するときに一度URLエンコードします。YouTubeのトークン自体に`%3D`などのパーセントエスケープが含まれる場合があります。リレーはクエリ文字列の解析後、追加で最大一層をデコードしてから検証・キャッシュ・上流へのポーリングを行います。不正なエスケープ、許可されないトークン文字、追加デコード前に8192文字を超えるトークンには400を返します。
