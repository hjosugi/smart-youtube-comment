...
残りの手動作業は、一度限りの Chrome Web Store デベロッパーダッシュボードのセットアップです：アイテムの作成、ストアリスティングのコピー/アセット、プライバシー/データ使用の宣言、サービスアカウントまたは OAuth 資格情報の設定、および GitHub シークレットの入力。その後、対応する `vX.Y.Z` タグがチェックを実行し、zip をビルドし、アップロードし、レビュー/公開のために提出できます。
