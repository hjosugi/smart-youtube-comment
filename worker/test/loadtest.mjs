...
async function sustained(seconds) {
  ...
  console.log(`    unique=${seen.size} rate=${(seen.size / elapsed).toFixed(1)} msg/s  batches=${batches.length} avg=${mean(batches)} max=${Math.max(...batches)} upstreamPolls=${upstream}`);
...
async function collapse(count, spreadMs) {
  ...
  console.log(`    HIT=${hits} MISS=${miss}  collapse=${((hits / count) * 100).toFixed(0)}%  (MISS = upstream calls)`);
  return { hits, miss };
}
...
async function spike(levels) {
  ...
  for (const c of levels) {
    ...
    const hits = res.filter((r) => r.cache === "HIT").length;
    console.log(`    c=${String(c).padStart(3)}  p50=${pct(lat, 50)}ms p95=${pct(lat, 95)}ms p99=${pct(lat, 99)}ms  errors=${errs}  cacheHIT=${hits}/${c}`);
  }
}
