...
async function main() {
  ...
  const done = client.start(videoId, {
    onMessages(msgs) {
      ...
      console.log(`[msgs] +${msgs.length} (total ${total}) e.g. "${msgs[0]?.text?.slice(0, 48) ?? ""}"`);
    },
    onState(s) {
  ...
  console.log(seen.size > 0 ? "RESULT: ✅ adaptive client polled live chat end-to-end" : "RESULT: ⚠️ no messages");
}
