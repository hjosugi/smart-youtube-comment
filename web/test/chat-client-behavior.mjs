...
globalThis.fetch = async (url) => {
  ...
  calls.push({ cont: u.searchParams.get("cont"), video: u.searchParams.get("video"), offset: u.searchParams.get("offset") });
  const env = {
...
// --- pause halts polling; resume continues ---
{
  calls = [];
  mode = "live";
  const c = createLiveChatClient({ base: "http://x", minIntervalMs: 20, quietThreshold: 0, jitterRatio: 0 });
  c.start("VIDEOIDXXXX", {});
  await sleep(140);
...
// --- replay mode sends the player offset on polls ---
{
  calls = [];
  mode = "replay";
  const c = createLiveChatClient({ base: "http://x", minIntervalMs: 20, quietThreshold: 0, getOffsetMs: () => 42000 });
  c.start("VIDEOIDXXXX", {});
  await sleep(140);
  c.stop();
  const contCalls = calls.filter((x) => x.cont);
  assert("replay: first poll resolves the video", calls[0].video === "VIDEOIDXXXX" && calls[0].offset === null);
  assert("replay: subsequent polls carry offset", contCalls.length >= 1 && contCalls.every((x) => x.offset === "42000"), JSON.stringify(contCalls.slice(0, 3)));
}
