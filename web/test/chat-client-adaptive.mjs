...
assert(
  "backoff grows 50<100<200",
  fails.length === 3 && fails[0] === 50 && fails[1] === 100 && fails[2] === 200,
  JSON.stringify(fails),
)
assert(
  "call sequence re-resolves after 3 fails",
  calls.join(",") === "video,cont,cont,cont,video,cont",
  calls.join(","),
)
assert(
  "recovered to healthy cadence 20ms",
  states.some(s => s.healthy && s.nextInMs === 20),
  JSON.stringify(states.filter(s => s.healthy).map(s => s.nextInMs)),
)
