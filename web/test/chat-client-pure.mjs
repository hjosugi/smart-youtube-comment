...
{
  const p = step({ cont: null, failures: 2, quiet: 0 }, {
    ok: true,
    env: { messages: [{ id: "a" }], continuation: "c1", timeoutMs: 5000, ended: false },
  });
  assert("healthy: advances cont + resets failures", p.state.cont === "c1" && p.state.failures === 0);
...
{
  const p = step({ cont: "c", failures: 0, quiet: 0 }, { ok: true, env: { messages: [{ id: "x" }], continuation: "c2", timeoutMs: 10 } });
...
{
  const a = step({ cont: "c", failures: 0, quiet: 0 }, { ok: true, env: { messages: [{ id: "x" }], ended: true } });
  const b = step({ cont: "c", failures: 0, quiet: 0 }, { ok: true, env: { messages: [], continuation: null } });
...
// non-empty poll snaps it back to the server cadence.
{
  ...
  const base = 10000;
  const q0 = qstep({ cont: "c", failures: 0, quiet: 0 }, { ok: true, env: { messages: [], continuation: "c", timeoutMs: base } });
  const q1 = qstep(q0.state, { ok: true, env: { messages: [], continuation: "c", timeoutMs: base } });
  const busy = qstep(q1.state, { ok: true, env: { messages: [{ id: "m" }], continuation: "c", timeoutMs: base } });
