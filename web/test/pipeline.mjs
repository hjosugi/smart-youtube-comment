...
{
  const p = render({ text: "that play was genuinely incredible, replay it", author: "@a", authorType: "member", kind: "text" });
  ...
  assert("payload carries no internal score leak beyond contract", p && "score" in p && "emphasis" in p && !("reasons" in p));
}
...
// renderBatch admits via the overlay and counts only accepted pushes
{
  ...
  assert("renderBatch returns admitted count", shown === pushed && shown >= 1, `shown=${shown} pushed=${pushed}`);
}
