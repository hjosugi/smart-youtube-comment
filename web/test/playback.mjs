...
// ---- makeFate: live (no offsetMs) ----
{
  ...
  assert("live: new message shows", fate({ id: "1", author: "@a", text: "hi" }, Infinity) === "show");
  seen.add("1");
  assert("live: duplicate id drops", fate({ id: "1", author: "@a", text: "hi" }, Infinity) === "drop");
  assert("NG author drops", fate({ id: "2", author: "@ng", text: "x" }, Infinity) === "drop");
  assert("NG word drops", fate({ id: "3", author: "@a", text: "a badword here" }, Infinity) === "drop");
  assert("NG checked before dedup", fate({ id: "1", author: "@ng", text: "x" }, Infinity) === "drop");
}
...
// future message is re-evaluated (skip doesn't consume it) and shows when due
{
  ...
  assert("replay: skip now then show when due", fate(m, 120000) === "skip" && fate(m, 130000) === "show");
}
...
assert("isSeek: long pause looks like seek -> true (app gates on `playing`)", isSeek(0, 3000) === true);
