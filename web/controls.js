...
const range = (spec, value, onInput) => {
  ...
  const input = tag(el("input", { type: "range", min: spec.min, max: spec.max, step: spec.step ?? 1, value }), spec.key);
...
const select = (spec, value, onInput) => {
  ...
  for (const o of spec.options) input.append(el("option", { value: o.value, textContent: o.label, selected: o.value === value }));
...
export const buildControl = (spec, value, onInput) => (BUILDERS[spec.type] ?? bool)(spec, value, onInput);
